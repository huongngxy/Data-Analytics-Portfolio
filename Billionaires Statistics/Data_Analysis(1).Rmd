...
Based on the normal curve, the distribution of the population's age is approximately normal since the most common age falls between 55 and 60, but not at the peak of the normal curve (between 60-65 or 70-75).\
...
df_agegroups_industries1 <- df_agegroups_industries %>% 
  ...
  summarise(anzahl = sum(Freq)) %>%
  mutate(anteil = anzahl/sum(anzahl)) %>%
  mutate(per = scales::percent(anteil, accuracy = 0.01))

plot_treemap <- df_agegroups_industries1  %>%
  ggplot(aes(area = anteil, fill = df_Billionaires.industries, 
...
```{r figure_10, echo=FALSE, fig.align="center", out.height= '80%', out.width='80%'}
