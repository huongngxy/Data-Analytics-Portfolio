...
miss_var_summary(Billionaires_Statistics_Dataset, order = TRUE)
```

```{r include=FALSE}
# Importing external data to fill NA in column gdp_country 
external_gdp_data <- read_csv("./external_gdp_data.csv")
external_gdp_data
```
...
```{r include=FALSE}
# count how many billionaires the listed countries with missing values have
...
```{r include=FALSE}
# merge GDP data from the external dataset to the main one 
...
The Finance & Investments industry has the highest number of billionaires (372). Other most populous sectors are Manufacturing (372), Technology (314), Fashion & Retail (266) and Food & Beverage (212).\
...
Technology is most populous sector in the top 10 since it has four billionaires named in the list including Jeff Bezos ($114 billion), Larry Ellison ($107 billion), Bill Gates ($104 billion) and Steve Ballmer ($80.7 billion).\

## 2. Crosstabulations

### 2.1 Net Worth and Industries
...
plot_netw_industry
```
Technology, Fashion & Retail, Finance & Investments, Manufacturing, and Food & Beverage represent the top five industries by aggregated net worth. This aligns with their high concentration of billionaires, as shown in Figure 1. 
...
plot_anova
``` 
The automotive sector leads with the highest average net worth ($7.20 billion) per billionaire. This differs from the patterns seen in Figures 1 and 7 because average net worth depends on the ratio between total industry wealth and number of billionaires in each sector.

### 2.2 Industries and Age groups 
...
A clear age-related trend emerges in the technology sector, with billionaire representation steadily declining across age groups (28%, 25.10%, 10.06%, 4.89%). This suggests younger billionaires are significantly more likely to have built their wealth in technology.\
Conversely, the Finance & Investment sector shows an increasing presence with age (0%, 13.62%, 14.33%, 14.53%), indicating this industry tends to produce billionaires later in their careers.
